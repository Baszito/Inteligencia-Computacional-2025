...
class AlgEvolutivo:
    def funcion_fitness_1(self, individuo: Genotipo):
        return -fg(individuo.get_x_numero() - 100, individuo.get_y_numero() - 100)
    ...
    def evolucion(self):
        ...
        if(it==self.max_iteraciones):
            print("No se llego a la aptitud requerida, iteraciones = " + str(it))
        else:
        ...
        print(self.mejor_individuo.get_y_numero()-100)
        print("f(x, y)")
        print(fg(self.mejor_individuo.get_x_numero()-100, self.mejor_individuo.get_y_numero()-100))
...
def f(x):
    return ((x[0]**2 + x[1]**2)**(0.25))*((math.sin(50*((((x[0]**2) + (x[1]**2))**0.1)))**2) + 1)
class Particula:
...
class Enjambre:
    def __init__(self, cant_particulas, dimensiones, min, max, const1, const2):
        ...
        self.y_global = copy.deepcopy(self.particulas[0].x)
    ...
    def loop_principal(self, max_it=1000, cuando_cortar = 0,crit_parada=50):
        ...
        while(it < max_it):
            ...
            if ((np.array(y_gl_anterior) == np.array(self.y_global)).all()):
                it_global += 1 
            else:
                it_global = 0
...
cant_particulas=500
criterio_parada=-0.5
poblacion=AlgEvolutivo(cant_particulas,16,criterio_parada,iteraciones)
poblacion.evolucion()
print("-----------------------------------------------------")
x_enjambre=Enjambre(cant_particulas,2,-100,100,1,2)
x_enjambre.loop_principal(1000, crit_parada=100)